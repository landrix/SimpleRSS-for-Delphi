...
iTunes is based off of RSS 2.0 with modifications
...
begin
  ...
  with FSimpleRSS.XMLFile do begin
    ...
    ChildNodes.Nodes[reRSS].DeclareNamespace(ituneNS,ituneNSURL);
  ...
  //START iTunes Support
  //itunes:author
  //<itunes:author>John Doe</itunes:author>
  FSimpleRSS.XMLFile.ChildNodes.Nodes[reRSS].ChildNodes.Nodes[itunesAuthor].NodeValue := FSimpleRSS.Channel.iTunes.Author;

  //itunes:subtitle
  //<itunes:subtitle>A show about everything</itunes:subtitle>
  FSimpleRSS.XMLFile.ChildNodes.Nodes[reRSS].ChildNodes.Nodes[iTunesSubtitle].NodeValue := FSimpleRSS.Channel.iTunes.Subtitle;
  ...
  FSimpleRSS.XMLFile.ChildNodes.Nodes[reRSS].ChildNodes.Nodes[iTunesSummary].NodeValue := FSimpleRSS.Channel.iTunes.Summary;
  ...
  //</itunes:owner>
  With FSimpleRSS.XMLFile.ChildNodes.Nodes[reRSS].ChildNodes.Nodes[iTunesOwner] do
    Begin
      ChildNodes.Nodes[itunesName].NodeValue := FSimpleRSS.Channel.iTunes.Owner.Name;
      ChildNodes.Nodes[itunesEmail].NodeValue := FSimpleRSS.Channel.iTunes.Owner.EmailAddress;
    end; // with do

  //itunes:image
  //iTunes prefers square images 300x300 pixels or larger
  //<itunes:image href="http://example.com/podcasts/everything/AllAboutEverything.jpg" />
  FSimpleRSS.XMLFile.ChildNodes.Nodes[reRSS].ChildNodes.Nodes[itunesImage].NodeValue := FSimpleRSS.Channel.iTunes.Image;
  ...
  //</itunes:category>
  With FSimpleRSS.XMLFile.ChildNodes.Nodes[reRSS].ChildNodes.Nodes[iTunesCategory] do
    Begin
      Attributes[itunesText] := FSimpleRSS.Channel.iTunes.Category.Category;
      If FSimpleRSS.Channel.iTunes.Category.SubCategory <> '' then // Subcategory is optional
        Begin
          With ChildNodes.Nodes[itunesCategory] do
            Attributes[itunesText] := FSimpleRSS.Channel.iTunes.Category.SubCategory;
        end; // if then
    end; // with do
  ...
  If FSimpleRSS.Channel.iTunes.Block then
    Begin
      XNode := FSimpleRSS.XMLFile.CreateNode(itunesBlock);
      FSimpleRSS.XMLFile.ChildNodes.Nodes[reRSS].ChildNodes.Add(XNode);
    end; //if then
  ...
  FSimpleRSS.XMLFile.ChildNodes.Nodes[reRSS].ChildNodes.Nodes[itunesExplicit].NodeValue := FSimpleRSS.Channel.iTunes.StrExplict;

  //<itunes:keywords>
  //This tag allows users to search on text keywords.
  //Use spaces to separate keywords.
  FSimpleRSS.XMLFile.ChildNodes.Nodes[reRSS].ChildNodes.Nodes[itunesKeywords].NodeValue := FSimpleRSS.Channel.iTunes.GetKeywordsAsString;

  //END iTunes Support
  // END CHANNEL
  // ITEMS
  for Counter := 0 to FSimpleRSS.Items.Count - 1 do begin
    if FSimpleRSS.Items.Items[Counter].TitleChanged or
      FSimpleRSS.Items.Items[Counter].DescriptionChanged then begin
      ...
      with Item do begin
        ...
        if FSimpleRSS.Items.Items[Counter].PubDate.Changed then
          ChildNodes.Nodes[rePubDate].NodeValue := FSimpleRSS.Items.Items[Counter].PubDate.GetDateTime;
        if FSimpleRSS.Items.Items[Counter].Source.Include then
          begin
            XNode := FSimpleRSS.XMLFile.CreateNode(reSource);
            XNode.NodeValue := FSimpleRSS.Items.Items[Counter].Source.Title;
            XNode.AttributeNodes.Nodes[reURL].NodeValue := FSimpleRSS.Items.Items[Counter].Source.URL;
            ChildNodes.Add(XNode);
          end; // if source
        //Begin iTunes Item Support
        // itunes:author
        // <itunes:author>John Doe</itunes:author>
        ChildNodes.Nodes[itunesAuthor].NodeValue := FSimpleRSS.Items.Items[Counter].iTunes.Author;

        // itunes:subtitle
        // <itunes:subtitle>A short primer on table spices</itunes:subtitle>
        ChildNodes.Nodes[iTunesSubtitle].NodeValue := FSimpleRSS.Items.Items[Counter].iTunes.Subtitle;
        ...
        ChildNodes.Nodes[iTunesSummary].NodeValue := FSimpleRSS.Items.Items[Counter].iTunes.Summary;

        // itunes:category
        // <itunes:category text="Food">
        With ChildNodes.Nodes[iTunesCategory] do
          Begin
            Attributes[itunesText] := FSimpleRSS.Items.Items[Counter].iTunes.Category.Category;
            If FSimpleRSS.Items.Items[Counter].iTunes.Category.SubCategory <> '' then // Subcategory is optional
              Begin
                With ChildNodes.Nodes[itunesCategory] do
                  Attributes[itunesText] := FSimpleRSS.Items.Items[Counter].iTunes.Category.SubCategory;
              end; // if then
          end; // with do

        // itunes:duration
        // <itunes:duration>7:04</itunes:duration>
        ChildNodes.Nodes[itunesDuration].NodeValue := FSimpleRSS.Items.Items[Counter].iTunes.Duration.Duration;

        // itunes:keywords
        // <itunes:keywords>salt pepper shaker exciting</itunes:keywords>
        ChildNodes.Nodes[itunesKeywords].NodeValue := FSimpleRSS.Items.Items[Counter].iTunes.KeyWords.Keywords;

        //<itunes:block>
        // Use this inside an <item> element to prevent that episode from appearing in the iTunes Podcast directory. Use this inside a <channel> element to prevent the entire podcast from appearing in the iTunes Podcast directory.
        If FSimpleRSS.Items.Items[Counter].iTunes.Block then
          Begin
            XNode := FSimpleRSS.XMLFile.CreateNode(itunesBlock);
            ChildNodes.Add(XNode);
          end; //if then
        ...
        // The two values for this tag are “yes” and “no”.
        // If you populate this tag with "yes", a parental advisory graphic will appear next to your podcast artwork on the iTunes Music Store, and in the Name column in iTunes.
        ChildNodes.Nodes[itunesExplicit].NodeValue := FSimpleRSS.Items.Items[Counter].iTunes.StrExplict;
        //End iTunes Item Support
      end; // with do
...
procedure TSimpleParseriTunes.Parse;
var
  Counter, SecondCounter: Integer;
  aTempNode: IXMLNode;
begin
  FSimpleRSS.Channel.iTunes.Block := False;
  with FSimpleRSS.XMLFile.DocumentElement.ChildNodes.Nodes[reChannel] do
    begin
      if ChildNodes.FindNode(reTitle) = nil then
        begin
          raise ESimpleRSSException.Create(emRequiredFieldMissing + reTitle);
        end; // if then
      if ChildNodes.FindNode(reLink) = nil then
        begin
          raise ESimpleRSSException.Create(emRequiredFieldMissing + reLink);
        end; // if then
      if ChildNodes.FindNode(reDescription) = nil then
        begin
          raise ESimpleRSSException.Create(emRequiredFieldMissing + reDescription);
        end; // if then
      if (ChildNodes.FindNode(reItem) <> nil) or (ChildNodes.FindNode(reCategory) <> nil) then
        begin
        ...
        if ChildNodes.Nodes[Counter].NodeName = reItem then begin
          with FSimpleRSS.Items.Add do begin
            iTunes.Block := False;
            if (ChildNodes.Nodes[Counter].ChildNodes.FindNode(reTitle) = nil) and (ChildNodes.Nodes[Counter].ChildNodes.FindNode(reDescription) = nil) then
              begin
...
//Begin iTunes Item Support
//itunes:author
            If ChildNodes.Nodes[Counter].ChildNodes.FindNode(itunesAuthor) <> nil then
              Begin
                iTunes.Author := ChildNodes.Nodes[Counter].ChildNodes.FindNode(itunesAuthor).NodeValue;
              end; // if then
//itunes:block
            If ChildNodes.Nodes[Counter].ChildNodes.FindNode(itunesBlock) <> nil then
              Begin
                iTunes.Block := True;
              end; // if then
//itunes:category
            If ChildNodes.Nodes[Counter].ChildNodes.FindNode(itunesCategory) <> nil then
              Begin
                iTunes.Category.Category := ChildNodes.Nodes[Counter].ChildNodes.FindNode(itunesCategory).NodeValue;
                If ChildNodes.Nodes[Counter].ChildNodes.FindNode(itunesCategory).ChildNodes.Count = 1 then
                  iTunes.Category.SubCategory := ChildNodes.Nodes[Counter].ChildNodes.FindNode(itunesCategory).ChildNodes.First.NodeValue;
              end; // if then
//itunes:explict
            If ChildNodes.Nodes[Counter].ChildNodes.FindNode(itunesExplicit) <> nil then
              Begin
                If ChildNodes.Nodes[Counter].ChildNodes.FindNode(itunesExplicit).NodeValue = itunesYes then
                  iTunes.Explict := True
                else
                  iTunes.Explict := False;
              end; // if then
//itunes:keywords
            If ChildNodes.Nodes[Counter].ChildNodes.FindNode(itunesKeywords) <> nil then
              Begin
                iTunes.SetKeywordsFromString(ChildNodes.Nodes[Counter].ChildNodes.FindNode(itunesKeywords).NodeValue);
              end; // if then
//itunes:subtitle
            If ChildNodes.Nodes[Counter].ChildNodes.FindNode(itunesSubtitle) <> nil then
              Begin
                iTunes.Subtitle := ChildNodes.Nodes[Counter].ChildNodes.FindNode(itunesSubtitle).NodeValue;
              end; // if then
//itunes:summary
            If ChildNodes.Nodes[Counter].ChildNodes.FindNode(itunesSummary) <> nil then
              Begin
                iTunes.Summary := ChildNodes.Nodes[Counter].ChildNodes.FindNode(itunesSummary).NodeValue;
              end; // if then
//End iTunes Item Support
...
//Begin iTunes Channel Support
//itunes:block
        If ChildNodes.Nodes[Counter].NodeName = itunesBlock then
          FSimpleRSS.Channel.iTunes.Block := True;
//itunes:category
        If ChildNodes.Nodes[Counter].NodeName = itunesCategory then
          Begin
            FSimpleRSS.Channel.iTunes.Category.Category := ChildNodes.Nodes[Counter].NodeValue;
            If ChildNodes.Nodes[Counter].ChildNodes.Count = 1 then
              FSimpleRSS.Channel.iTunes.Category.SubCategory := ChildNodes.Nodes[Counter].ChildNodes.First.NodeValue;
          end; // if then
//itunes:explict
        If ChildNodes.Nodes[Counter].NodeName = itunesExplicit then
          Begin
            If ChildNodes.Nodes[Counter].NodeValue = itunesYes then
              FSimpleRSS.Channel.iTunes.Explict := True
            else
              FSimpleRSS.Channel.iTunes.Explict := False;
          end; // if then
//itunes:keywords
        If ChildNodes.Nodes[Counter].NodeName = itunesKeywords then
          Begin
            FSimpleRSS.Channel.iTunes.SetKeywordsFromString(ChildNodes.Nodes[Counter].NodeValue);
          end; // if then
//itunes:owner
        If ChildNodes.Nodes[Counter].NodeName = itunesOwner then
          Begin
            FSimpleRSS.Channel.iTunes.Owner.Name := ChildNodes.Nodes[Counter].ChildNodes.Nodes[itunesName].NodeValue;
            FSimpleRSS.Channel.iTunes.Owner.EmailAddress := ChildNodes.Nodes[Counter].ChildNodes.Nodes[itunesEmail].NodeValue;
          end; // if then
//itunes:subtitle
        If ChildNodes.Nodes[Counter].NodeName = itunesSubtitle then
          Begin
            FSimpleRSS.Channel.iTunes.Subtitle := ChildNodes.Nodes[Counter].NodeValue;
          end; // if then
//itunes:summary
        If ChildNodes.Nodes[Counter].NodeName = itunesSummary then
          Begin
            FSimpleRSS.Channel.iTunes.Summary := ChildNodes.Nodes[Counter].NodeValue;
          end; // if then
//itunes:image
        If ChildNodes.Nodes[Counter].NodeName = itunesImage then
          Begin
            FSimpleRSS.Channel.iTunes.Image := ChildNodes.Nodes[Counter].NodeValue;
          end; // if then
//End iTunes Channel Support
